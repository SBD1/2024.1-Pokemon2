...
-- Tabela de tipo pokemon
CREATE TABLE tb_pokemon_tipo (
    id_pokemon INT REFERENCES tb_pokemon(id_pokemon),
    id_tipo INT REFERENCES tb_tipo(id_tipo),
...
-- Tabela Tipo de Ataque
CREATE TABLE tb_ataque_tipo (
    id_ataque INT REFERENCES tb_ataque(id_ataque),
    id_tipo INT REFERENCES tb_tipo(id_tipo),
    PRIMARY KEY (id_ataque, id_tipo)
);
